...
MOSSTTSKit 现在有 `TextNormalizer`，先处理省略号、连续破折号、换行、结尾悬空标点这类确定会影响稳定性的输入。
...
这就是我们把省略号、连续破折号、换行、结尾冒号沉淀到 `TextNormalizer` 的原因。
...
我们在 MOSSTTSKit 里新增 `TextNormalizer`，就是因为 MOSS-TTS-Nano 对省略号、连续破折号、换行、结尾悬空标点比较敏感。
